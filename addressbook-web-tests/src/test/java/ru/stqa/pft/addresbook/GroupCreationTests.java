...
public class GroupCreationTests {
  FirefoxDriver wd; //вд - переменная, которая является аттрибутом обьекта типа GroupCreationTests.
  ...
  public void setUp() throws Exception { //тестовый ФВ выполняет в обьекте метод Сетап
    ...
    login("admin", "secret"); //вызов созданного метода

  }

  private void login(String username, String password) { //создаем метод логин
  ...
  public void testGroupCreation() {
    gotoGroupPage();
    initGroupCreation();
    fillGroupForm("Testgroup1", "Testgroup2", "Testgroup3");
    submitGroupCreation();
    returnToGroupPage();
  }
